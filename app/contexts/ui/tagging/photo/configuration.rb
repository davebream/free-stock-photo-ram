    ...
    class Photo
      class Configuration
        ...
        def with_photo(event)
          UI::Tagging::Photo.find_or_initialize_by(id: event.data.fetch(:photo_id)).tap do |photo|
            yield photo
            photo.save!
          end
        end
        ...
        def link_to_stream(event)
          @cqrs.link_event_to_stream(event, "UI::Tagging::Photo$#{event.data.fetch(:photo_id)}")
        end
      end
    end
  end
